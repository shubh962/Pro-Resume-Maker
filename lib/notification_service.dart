...
// - 40+ unique CTA messages, never repeats the same one twice in a row
// - 1 notification per day, at a random time between 9 AM – 8 PM
...
class NotificationService {
  ...
  Future<void> scheduleDaily() async {
    ...
    final rng  = Random();
    final hour = 9 + rng.nextInt(12);   // 9–20
  ...
  Future<Map<String, String>> _nextMessage() async {
    ...
    // Reshuffle when we've gone through all messages
    if (lastIndex == 0) {
      order.shuffle(Random());
    }

    await prefs.setStringList(_kShuffleOrder, order.map((e) => e.toString()).toList());
